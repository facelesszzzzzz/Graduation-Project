...
void MainRunning_Task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        lKeyAnswer = KEY_Scan(0) ? KEY_Scan(0) : lRecKey;          //硬件按键响应优先级最高
        switch(lKeyAnswer)
        {
            case KEY0_PRES:
                /* 模式切换 */
                lCurrentMode = lCurrentMode == DEFAULT_MODE ? CARE_MODE : DEFAULT_MODE;
                if(CARE_MODE == lCurrentMode)
            ...
            case CARE_MODE:
                ...
                if(SOUND_READ){
                    ...
                    if(lSoundCount > 3 && VOICE_DISABLE == lSoundState){
                        ...
                        if(ESP8266_EventGroup_Handle != NULL)
                            xEventGroupSetBits(ESP8266_EventGroup_Handle, lVoicePlay[ESP8266_FIRST_BIT]);
                    }
                }else{
